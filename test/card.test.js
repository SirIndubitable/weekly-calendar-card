...
test("CalendarEvent._getEventClass", () => {
  ...
  let originalStart =  DateTime.now().minus({ days: 2 });
  let originalEnd = DateTime.now().plus({ days: 5 });
...
test("CalendarEvent.renderSummary(event)", () => {
  let start = DateTime.local(2024, 10, 1, 11, 47);
  let event = CreateEvent(start, start.plus({ hours: 1 }));
...
test("CalendarEvent.renderSummary(multiDayEvent)", () => {
  ...
  let eventStart = originalStart.plus({ days: 2 }).startOf("day");
  event = CreateEvent(
    originalStart,
    originalEnd,
    eventStart,
...
test("CalendarEvent.Build(MultiDayEvent)", () => {
  let start = DateTime.local(2024, 10, 1, 15, 30);
  let end = DateTime.local(2024, 10, 3, 16, 0);
  let eventData = {
    summary: "Test Event",
  ...
  let events = CalendarEvent.Build(config, calendar, eventData);
...
test("CalendarEvent.Build(FullDayEvent)", () => {
  let start = DateTime.local(2024, 10, 1);
  let end = DateTime.local(2024, 10, 2);
  let eventData = {
    summary: "Test Event",
    start: { dateTime: start.toISO() },
    end: { dateTime: end.toISO() },
...
test("CalendarEvent.Build(SimpleEvent)", () => {
  let start = DateTime.local(2024, 10, 1, 9, 45);
  let end = DateTime.local(2024, 10, 1, 10, 0);
  let eventData = {
    summary: "Test Event",
    start: { dateTime: start.toISO() },
    end: { dateTime: end.toISO() },
...
test.each(Array(10).fill(null))("CalendarEvent.compareTo", () => {
  ...
  let originalStart = DateTime.local(2020, 2, 14, 11, 30);
  event1 = createEvent({ multiDay: false, fullDay: false, originalStart: originalStart, start: DateTime.local(2020, 2, 14, 11, 30) });
  event2 = createEvent({ multiDay: false, fullDay: false, originalStart: originalStart, start: DateTime.local(2020, 2, 14, 11, 45) });
  expect([event1, event2].sort(CalendarEvent.compareTo)[0]).toBe(event1);
  expect([event2, event1].sort(CalendarEvent.compareTo)[0]).toBe(event1);

  event1 = createEvent({ multiDay: false, fullDay: false, originalStart: originalStart, start: originalStart, calendarSorting: 1 });
  event2 = createEvent({ multiDay: false, fullDay: false, originalStart: originalStart, start: originalStart, calendarSorting: 2 });
  expect([event1, event2].sort(CalendarEvent.compareTo)[0]).toBe(event1);
  expect([event2, event1].sort(CalendarEvent.compareTo)[0]).toBe(event1);

  event1 = createEvent({ multiDay: false, fullDay: false, originalStart: originalStart, start: originalStart, calendarSorting: 1, summary: "A" });
  event2 = createEvent({ multiDay: false, fullDay: false, originalStart: originalStart, start: originalStart, calendarSorting: 1, summary: "B" });
  expect([event1, event2].sort(CalendarEvent.compareTo)[0]).toBe(event1);
  expect([event2, event1].sort(CalendarEvent.compareTo)[0]).toBe(event1);
});
